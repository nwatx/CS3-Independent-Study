...
struct Seg { // comb(ID,b) = b
	...
	T comb(T a, T b) {
		return {
			a.sum + b.sum,
			max(a.sum + b.pref, max(a.pref, a.sum)),
			max(a.suf + b.sum, max(b.sum, b.suf)),
			max(a.sum + b.sum, max(a.suf + b.sum, max(b.pref + a.sum, max(a.sum, b.sum))))
		};
	} 
	int n; vector<T> seg;
...
int main() {
	...
	F0R(i, n) {
		ll a; re(a);
		seg.upd(i, {a, a, a, max(0LL, a)});
	}

	each(se, seg.seg) {
		dbg(se.pref, se.sum, se.suf);
	}

	pr(seg.query(0, n-1).maxSum, nl);

	F0R(i, m) {
		int a, b;
		re(a, b);
		seg.upd(a, {b, b, b, max(0,b)});
		pr(seg.query(0, n-1).maxSum, nl);
	}
