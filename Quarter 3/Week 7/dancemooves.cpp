...
void dfs(int x) { // if we have reached the cycle merge the two sets
	if(vis[x]) return;
	path.pb(x);
	// each(e, items[x]) cycle.insert(e);
	vis[x] = true;
...
int main() {
	...
	F0R(i, n) { P[i] = i; items[i].insert(i); }
	...
	F0R(i, n) {
		...
		each(e, path) {
			each(item, items[e]) p.insert(item);
		}

		each(idx, path) A[idx] = sz(p);
		path = vi();
		// while(sz(path)) {
		// 	A[path.front()] = sz(cycle);
		// }
	}
