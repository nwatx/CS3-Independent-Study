...
// 	if(k == 1) return F[x];
// 	else return solve(solve(x, k/2), k/2);
// }
...
int main() {
	...
	each(e, swaps) {
		F0R(j, (e.s-e.f+1)/2) {
			// dbg(e, j);
			assert(F[e.f+j] && F[e.s-j]);
			swap(F[e.f+j], F[e.s-j]);
		}
		// dbg(e);
		// FOR(i, 1, N+1) dbg(F[i]);
	}
	...
	FOR(i, 1, N+1) {
		if(!vis[i]) {
			...
			int mod = K % sz(cycle);
			F0R(j, sz(cycle)) F[cycle[j]] = cycle[(j+mod)%sz(cycle)];
		}
	}
	...
	// FOR(i, 1, N+1) dbg(F[i]);
	// FOR(i, 1, N+1) dbg(solve(i));
	// buildSucc();
	// F0R(i, 33) {
	// 	F0R(j, N+1) cerr << dp[i][j] << " ";
	// 	cerr << "\n";
	// }

	// FOR(i, 1, N+1) pr(solve(i), nl);
