...
// to determine whether a ratio y is valid, use knapsack!
bool valid(ll y) {
	vl dp(w+1, -INF); // dp[i] = most ATS with i cows

	dp[0] = 0;

	F0R(i, n) {
		ll value = 1000 * T[i] - y * W[i];
		R0F(j, w+1) {
			ll capV = min(w, j + W[i]);
			if(dp[j] == -INF) continue;
			ckmax(dp[capV], dp[j] + value);
		}
	}
