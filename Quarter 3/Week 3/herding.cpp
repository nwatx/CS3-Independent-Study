...
template <class T>
using ost = tree<T, null_type, less<T>, rb_tree_tag, tree_order_statistics_node_update>; //order statistic tree!
...
int solve()
{
    // we are trying to find the longest sequence with less than an diff - 1 gap
...
int main()
{
    ...
    F0R(i, N) re(A[i]);
    sort(A, A + N);
    pr(solve(), nl, max(A[N-2]-A[0], A[N-1]-A[1]) - (N-2));
