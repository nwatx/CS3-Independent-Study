...
int main() {
	...
	setIO();
    re(N);
    F0R(i, N) F0R(j, N) re(compat[i][j]);
    // we choose to fix i;
    F0R(i, N) if(compat[0][i]) dp[1 << i] = 1;
    FOR(s, 1, 1 << N) { // for every subset of males
        int v = __builtin_popcount(s);
        F0R(x, N) { // for every male
            if((s & (1 << x)) || !dp[s] || !compat[v][x]) continue;
            dp[s|(1<<x)] += dp[s];
            dp[s|(1<<x)] %= MOD;
        }
    }
